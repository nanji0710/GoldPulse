# 持仓域增强 + 后台提醒落地 设计文档（增量版）

> 日期：2026-08-04（v2 增量修订）
> 来源：brainstorming 流程（用户选定范围 + 首页呈现形式 + 明确"在原有基础上增加，不替换"）
> 关联计划：docs/superpowers/plans/2026-08-04-app-redesign-v2.md

## 0. 增量原则（重要）

**本次全部在原有功能基础上"增加"，不替换、不删除任何现有能力。**

### 保持不变（原有功能）
- 首页：三张行情卡（Au9999 / 浙商 / 工商，含状态胶囊、数据源标注、更新时间）
- 行情页：三品种切换、区间统计、当日统计、双轴折线/K线图表、刷新
- 资产页：原有持仓列表、长按菜单（修改克重/卖出/生息/删除）
- 提醒页、设置页（刷新频率/备份/清空）、引导页
- 黑金主题、收盘持续刷新、多源降级、三口径收益公式

### 新增（在原有基础上增加）
| 新增项 | 说明 |
|---|---|
| 首页收益区：每品种一张卡 + 全部合计卡 | 原收益区只显示"第一笔持仓"（bug），改为按品种展示——这是**扩充**收益区，不动行情卡 |
| 资产页顶部「持仓汇总」块 | 原有列表下方/上方新增一个汇总卡 |
| 持仓详情页（新页面） | 点持仓进入，展示交易流水 + 收益 + 操作 |
| 追加买入操作 | 长按菜单 + 详情页新增入口 |
| 后台提醒落地 | WorkManager 真正执行后台抓取+判定+通知（原来只是空壳） |

### 唯一"调整"而非"替换"
首页收益区：从「1 张卡（仅第一笔持仓）」→「每品种 1 张 + 全部合计」。这是对既有 bug 的修复与扩充，行情卡区不受影响。
