...
## Task 11（修订）: 收盘后自动刷新失效修复 + 新浪源核查

**用户反馈**：SGE 收盘后手动刷新金价仍变化（银行积存金收盘后价格持续变动），但自动刷新失效。

**根因**：三个行情轮询器有 `if (firstRun || isTradingNow() || last == null)` 交易时段门控——
收盘后自动轮询被暂停，只有手动刷新（触发 firstRun）才拉取。

**修复**：去掉 `isTradingNow()` 门控，三个轮询器改为按设定间隔（1s/5s/10s/...）持续拉取并入库；
`已收盘` 仅作为状态胶囊信息展示，不再暂停自动刷新；30s 快速重试（无缓存时）保持不变。
（伦敦金方案经用户否决已移除；新浪源核查结论见下。）

**新浪源核查结论（2026-08-04 实测）**：hq.sinajs.cn 对 `shau9999`/`au9999` 返回空、
`AU0` 沪金期货为 2024 年陈旧数据——该网络下新浪对三个品种无有效实时数据，无法作为切换源。
京东接口本身不"收盘"（积存金价格收盘后持续变动），持续轮询即可保持实时；
真实京东故障时仍走既有降级链（京东 → 东方财富 Au9999 参考 → 新浪 → 缓存）。

验收：收盘时段自动刷新持续产生入库日志；全部测试通过。
